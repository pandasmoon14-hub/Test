...
CLOSURE_MERGE="c14da427bf5c5c21c7ef1655e83aea3519587cc6"
...
def test_src_merge_bookkeeping_is_exact():
 m=load_at(CLOSURE_MERGE,MAN); by={x["workstream_id"]:x for x in m["workstreams"]}; s=by["PR2-SRC"]; tr={x["tranche_id"]:x for x in s["planned_tranches"]}
...
def test_review_is_complete_and_still_nonauthorizing():
 r=load_at(CLOSURE_MERGE,REVIEW)
...
def test_dependency_release_was_readiness_only_at_closure():
 m=load_at(CLOSURE_MERGE,MAN); by={x["workstream_id"]:x for x in m["workstreams"]}
...
def test_cross_recording_preserves_closure_boundary():
 p=read_at(CLOSURE_MERGE,PROG); d=read_at(CLOSURE_MERGE,DEC)
