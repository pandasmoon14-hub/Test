...
This module is test infrastructure only. It records the exact runtime-domain
package surface accepted by package guardrails. It grants no runtime authority
and must never be imported by ``src/astra_runtime``.
"""
...
# Historical guardrails that inspect raw directory entries also see this
# generated cache directory after Python imports the package.
AUTHORIZED_RUNTIME_DOMAIN_ENTRIES = frozenset(
    {*AUTHORIZED_RUNTIME_DOMAIN_FILES, "__pycache__"}
)
...
def runtime_domain_entries(repo_root: Path) -> frozenset[str]:
    """Return all current entries in ``astra_runtime.domain``."""

    domain_dir = repo_root / "src" / "astra_runtime" / "domain"
    return frozenset(path.name for path in domain_dir.iterdir())
