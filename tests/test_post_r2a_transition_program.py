...
def test_r4_d_implementation_post_merge_closure_control_state_is_exact():
    ...
    assert target["implementation_pull_request"] == 437

    assert (
        target["implementation_branch_head"]
        == "15f26809706981a13bc4183fa2ace5209b2152a2"
    )

    assert (
        target["implementation_merge_commit"]
        == "2b208838f91ac7b8488ade6287c51ee3ecb175e2"
    )

    assert target["implementation_merge_tree"] == "a9d5a22fa558391a037659d00fc6f5325ddb5467"

    assert target["post_merge_closure_complete"] is True
    assert target["post_merge_closure_regression_certified"] is True
...
def test_r4_d_post_merge_closure_root_seam_recovery_control_is_exact():
    manifest = _load_manifest()
    ...
    assert (
        recovery[
            "matching_transition_control_stale_expectation_detected"
        ]
        is True
    )
...
def test_r4_e_object_custody_transfer_definition_is_bounded():
    package_path = ROOT / "docs" / "doctrine" / "reviews" / "r4_e_persistent_world_object_custody_transfer_package.yaml"
    decision_path = ROOT / "docs" / "decisions" / "current_decisions_log.md"
...
def test_r4_e_implementation_post_merge_target_lifecycle_is_exact():
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    target = manifest["r4_e_object_custody_transfer_target"]
    ...
    assert target["implementation_cross_platform_ci_certified"] is True
    assert target["post_merge_closure_complete"] is True
    if target["post_merge_closure_regression_certified"] is False:
        assert target["post_merge_closure_recording_state"] == "pending_regression_certification"
        assert target["next_gate"] == "r4_e_post_merge_closure_regression_certification"
        assert target["next_gate_authorized"] is True
    else:
        assert target["post_merge_closure_recording_state"] == "regression_certified_pending_commit"
        assert target["next_gate"] == "r4_e_post_merge_closure_commit_push"
        assert target["next_gate_authorized"] is False
    assert target["r4_activation_authorized"] is False
    assert target["runtime_promotion_authorized"] is False
...
def test_r4_e_implementation_authorization_is_bounded_and_closure_merge_recorded():
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    target = manifest["r4_e_object_custody_transfer_target"]
    assert target["implementation_authority_effect"] == "bounded_persistent_world_object_custody_transfer_implementation_only"
    assert target["implementation_starting_baseline"] == "55461eb5ab9ac373cca9fa7b978ce71d0a11669c"
    assert target["implementation_starting_tree"] == "56fb1312b2857900a9864246189837aa68284777"
    assert target["definition_post_merge_closure_record_pull_request"] == 441
    assert target["definition_post_merge_closure_record_branch_head"] == "b78e9adc6b3e5dc34594d9e9b019859b086e57fa"
    assert target["definition_post_merge_closure_record_ci_run"] == 284
