...
def test_r4_e_definition_post_merge_closure_target_tracks_certification_state():
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    target = manifest["r4_e_object_custody_transfer_target"]
    if target["definition_post_merge_closure_regression_certified"] is False:
        assert target["definition_post_merge_closure_recording_state"] == "pending_regression_certification"
        assert target["next_gate"] == "r4_e_definition_post_merge_closure_regression_certification"
        assert target["next_gate_authorized"] is True
    else:
        assert target["definition_post_merge_closure_recording_state"] == "regression_certified_pending_commit"
        assert target["next_gate"] == "r4_e_definition_post_merge_closure_commit_push"
        assert target["next_gate_authorized"] is False
    assert target["implementation_authorized"] is False
    assert target["r4_activation_authorized"] is False
    assert target["runtime_promotion_authorized"] is False
