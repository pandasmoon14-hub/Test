...
def test_r4_e_object_custody_transfer_definition_is_bounded():
    package_path = (
        ROOT / "docs" / "doctrine" / "reviews"
        / "r4_e_persistent_world_object_custody_transfer_package.yaml"
    )
    ...
    assert target["package_id"] == "R4-E"
    assert target["package_name"] == "persistent_world_object_custody_transfer"
    ...
    assert package["custody_semantics"]["semantic_owner"] == "RT-010"
    assert package["semantic_owner_boundary"]["storage_or_serialization_transfers_semantic_ownership"] is False
