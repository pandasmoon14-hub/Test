...
ACCEPTED_HEAD="4aa1fce6a74f97b275a9c1d5975d0d192dcd2506"
...
def test_exact_committed_scope_and_ancestry():
 ...
 assert subprocess.run(["git","merge-base","--is-ancestor",ACCEPTED_HEAD,"HEAD"]).returncode==0
 ...
 changed=set(subprocess.check_output(["git","diff","--name-only",f"{BASE}...{ACCEPTED_HEAD}"],text=True).splitlines())
 assert changed==ALLOW
 assert subprocess.run(["git","diff","--check",f"{BASE}...{ACCEPTED_HEAD}"],capture_output=True).returncode==0
 num=subprocess.check_output(["git","diff","--numstat",f"{BASE}...{ACCEPTED_HEAD}"],text=True); assert "-\t-\t" not in num
 assert not subprocess.check_output(["git","diff","--name-status","--diff-filter=D",f"{BASE}...{ACCEPTED_HEAD}"],text=True).strip()
...
def test_counts_actions_limits_and_gate():
 ...
 gate=subprocess.check_output(["git","show",f"{ACCEPTED_HEAD}:{PLAN.relative_to(ROOT)}"],text=True); assert all(x in gate for x in ("`R1=complete`","`R2=active_incomplete`","`R2-0=complete`","`R2A=ready`","`R2B=blocked`","`R2C=blocked`","`R3–R6=blocked`","`RT-002G=unauthorized`","`temporary_evidence_deletion=unauthorized`"))
