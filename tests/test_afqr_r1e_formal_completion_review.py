...
def test_all_r1c_edge_hashes_fields_and_r1d_projections():
 ...
 for eid,x in rows.items():
  ...
  got={(z['family'],z['record_sha256']) for z in x['applicable_r1d_projection_records']};assert got=={(n,sha(r)) for n,r in pj[eid]}
  for n,r in pj[eid]:
...
def test_collisions_both_ledgers_evidence_paths_and_pressure_cases():
 ...
 for cid,d in ds.items():
  assert d['terms']==b[cid]['terms'];assert d['affected_afqrs']==b[cid]['affected_afqrs'];assert d['r1b_owner_candidates']==b[cid]['current_owner_candidates'];assert d['r1b_evidence_records']==b[cid]['source_evidence_records'];assert d['r1d_candidate_record']['record_sha256']==sha(cand[cid]);assert d['r1d_candidate_record']['exact_fields']==cand[cid];assert len(d['corpus_scale_pressure_test'])>=minimum[cid];assert d['alternatives_considered'] and d['lawful_qualified_forms'] and d['prohibited_inferences'] and d['handoff_rules'] and d['residual_r2_questions']
  ...
  bundle=r[cid]['resolution_evidence'];assert bundle['r1b_evidence_identifiers'] and bundle['r1c_invariant_ids'] and bundle['r1c_dependency_edge_ids'] and bundle['primary_evidence_identifiers'];[check_locator(z) for z in bundle['r1b_evidence_locators']+bundle['primary_evidence_locators']]
 assert len({b[x]['resolution_summary'] for x in b})==3
