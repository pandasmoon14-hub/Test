# AFQR-01–20 temporary physical-source intake

This tree preserves the twelve immutable ratification ZIPs received at `43a50c3756c4f1ba335956d8224f22b34fc32ab5` at their original base-branch paths under `incoming/`. `manifest.yaml` is the provenance ledger for every one of the 277 archive members: SHA-256, byte size, archive integrity, path-safety checks, classification, primary selection, and materialization status are content-backed.

## Selective textual materialization

Only 22 UTF-8, NUL-free source members with a defined R1 consolidation purpose are committed under `extracted/`: the twenty selected primary decision texts, the corrected AFQR-14 baseline note, and the superseded AFQR-14 manifest evidence. The other 255 members remain fully inventoried but archive-only (2 binary and 253 unsupported/redundant for PR materialization). Their unchanged parent ZIP is the immutable physical evidence. No textual reconstruction was invented.

## Authority boundary

Everything here is **temporary, non-authoritative working evidence**. It grants no canon, runtime, conversion, implementation, sourcebook, model, or live-play authority. Production code must not import this tree. The `incoming/` archives must remain byte-identical to the base. Analysis rejected absolute paths, traversal, symlinks, duplicates, encryption, unreadable members, and overwrite. `superseded/` and `unresolved/` are reserved ledgers, not deletion queues.

AFQR-14 uses the corrected normative package: AFQR-15's physical correction note says not to rely on the stale AFQR-14 manifest and validates the normative files directly. No competing package was silently combined.

R1A is complete only as intake/planning. R1 remains incomplete. Deletion requires an explicit later R1E exit gate proving durable provenance retention and reviewer approval. R2–R6 and RT-002G remain blocked and unauthorized.
