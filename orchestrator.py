...
def expand_chunk_markers(content: str) -> str:
    # Production rule: do not fabricate page markers from chunk ranges.
    return content
...
def process_book(cfg: RuntimeConfig, pdf: Path, repair_queue: dict[str, Any]) -> BookManifest:
    ...
    if "<!-- CHUNK:" in txt and ("<!-- PAGE:" not in txt or page_marker_mode == "chunk_fallback"):
        # diagnostic-only expansion path; must remain untrusted
    ...
    for i, pdf in enumerate(pdfs, start=1):
        ...
        try:
            ...
            if db is not None:
                ...
                if retry_count >= 5:
                    db.mark_terminal(book_key, str(exc), failure_class="orchestrator_error")
                else:
